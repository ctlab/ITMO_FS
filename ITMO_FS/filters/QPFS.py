...
class QPFS(object):
	...
	def run(self, X, y, r = None, sigma = None, solv='quadprog', fn=mutual_info_score):#TODO find the r and sigma values to be set as default
		"""
		...
		priors = QPFS.__count_priors(y) # Count prior probabilities of classes
		for i in range(class_size): # Loop through classes
			Ck = QPFS.__getCk(y, i) # Get array C(i) where C(k) is 1 when i = k and 0 otherwise
		...
		alpha = QPFS.__countAlpha(A, B, F) # Only in filter method, in wrapper we should adapth it based on performance
		...
		eigvalsFilt, UFilt = QPFS.__filterBy(sigma, eigvals, U) # Take onyl eigenvalues greater than threshold and corresponding eigenvectors
