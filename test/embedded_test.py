...
class TestCases(unittest.TestCase):
    ...
    def test_MOS_no_sampling(self):
        # MOSS
        res = MOS(model=SGDClassifier(), weight_func=weight_func).fit_transform(
    ...
    def test_MOSS(self):
        # MOSS
        res = MOS(
            model=SGDClassifier(), weight_func=weight_func, sampling=True
        ).fit_transform(self.data, self.target)
        assert self.data.shape[0] == res.shape[0]
        print("MOSS:", self.data.shape, "--->", res.shape)
    ...
    def test_MOSS_hinge(self):
        # MOSS
        res = MOS(
            model=SGDClassifier(), weight_func=weight_func, sampling=True, loss="hinge"
        ).fit_transform(self.data, self.target)
        assert self.data.shape[0] == res.shape[0]
        print("MOSS:", self.data.shape, "--->", res.shape)
    ...
    def test_MOSNS(self):
        # MOSNS
        res = MOS(
            model=SGDClassifier(), weight_func=weight_func, sampling=False
        ).fit_transform(self.data, self.target)
        assert self.data.shape[0] == res.shape[0]
    ...
    def test_losses(self):
        for loss in ["log", "hinge"]:
            res = MOS(
                model=SGDClassifier(), weight_func=weight_func, loss=loss
            ).fit_transform(self.data, self.target)
            assert self.data.shape[0] == res.shape[0]
    ...
    def test_pipeline(self):
        ...
        p = Pipeline(
            [
                (
                    "FS1",
                    MOS(model=SGDClassifier(), weight_func=weight_func, loss="log"),
                ),
                (
                    "FS2",
                    MOS(model=SGDClassifier(), weight_func=weight_func, loss="hinge"),
                ),
            ]
        )
        p.fit(self.data, self.target)
