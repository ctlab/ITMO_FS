...
class TestCases(unittest.TestCase):
    ...
    def test_filters(self):
        ...
        res = UnivariateFilter(
            spearman_corr, select_best_by_value(0.9999)
    ...
    def test_k_best(self):
        ...
        for i in [5, 10, 20]:
            res = UnivariateFilter(spearman_corr, select_k_best(i)).fit_transform(
                data, target
            )
    ...
    def test_corr(self):
        ...
        for f in [spearman_corr, pearson_corr, fechner_corr, kendall_corr]:
            ...
            np.testing.assert_array_almost_equal(
                np.round(f(data[:, 0].reshape((-1, 1)), data[:, 0]), 10),
                np.array([1.0]),
            )
            # X.shape == 2 case
            res = f(data, target)
    ...
    def test_corr_verification(self):
        # Values verification
        data, target = (
            np.array([[1, 8], [3, 2], [4, 5], [0, 7], [7, 1], [8, 4]]),
            np.array([9, 3, 4, 6, 2, 1]),
        )
        for f, verif in zip(
            [spearman_corr, pearson_corr], [stats.spearmanr, stats.pearsonr]
        ):
            true_res = []
    ...
    def test_modified_t_score_by_hand_small(self):
        X = np.array(
            [[5, 1, 3, 2], [4, 2, 2, 1], [3, 3, 4, 1], [2, 2, 3, 1], [1, 1, 5, 2]]
        )
    ...
    def test_mi(self):
        ...
        eq_4 = (
            entropy(list(zip(X_j, y)))
            - conditional_entropy(X_j, y)
            - conditional_entropy(y, X_j)
        )
        print(eq_1, eq_2)
    ...
    def test_gini_index(self):
        X = np.array(
            [[1, 2, 3], [-1, 2, 3], [1, 2, 2], [-1, 1, 1], [1, 2, 1], [-1, 3, 4]]
        )
    ...
    def test_pipeline(self):
        ...
        p = Pipeline(
            [("FS1", NDFS(3)), ("FS2", UnivariateFilter("PearsonCorr", ("K best", 1)))]
        )
        ...
        p = Pipeline(
            [
                ("FS1", UnivariateFilter("FechnerCorr", ("K best", 3))),
    ...
    def test_est(self):
        for f in [
            UnivariateFilter("FechnerCorr", ("K best", 2)),
            NDFS(2),
            RFS(2),
            SPEC(2, gamma=np.square),
        ]:
            check_estimator(f)
