...
class MOS(BaseTransformer):
    """
        Performs Minimizing Overlapping Selection under SMOTE (MOSS) or under
        ...
        threshold : float, optional
            The threshold value for feature dropout. Instead of comparing them
            to zero, they are normalized and values with absolute value lower
            than the threshold are dropped out.
        epochs : int, optional
        ...
        sampling : bool, optional
            Bool value that control whether MOSS (True) or MOSNS (False) should
            be executed.
    ...
    def _fit(self, X, y, **kwargs):
        ...
        if self.loss not in ['hinge', 'log']:
            raise KeyError(
                "Loss should be 'hinge' or 'log', %r was passed" %
                self.loss)
        ...
        if self.sampling:
            try:
                X, y = SMOTE(
                    ...
                    k_neighbors=self.k_neighbors).fit_resample(
                    X, y)
            except ValueError as e:
                print(
                    'Couldn\'t perform SMOTE because n_neighbors is bigger '
                    'than amount of instances in one of the classes.')
                raise e
