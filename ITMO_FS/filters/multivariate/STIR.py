...
class STIR(BaseTransformer):
    ...
    def distance_matrix(self, X):
        ...
        # max_diff_vec = self.max_diff(X) # TODO: probably should convert to
        #  float64 and add division by zero check
        max_diff_vec = self._max_diff(X).astype('float64')
    ...
    def find_neighbors(self, X, y):
        """
            ...
            hitmiss : array-like, shape (2, )
                ...
                Each list has two columns: index is the first column
                (instances)in both lists. The second column is hit_index
                (nearest hits for the first column instance) for list [1] and
                miss_index (nearest misses) for list [2].
        """
        ...
        for i in range(num_samples):
            ...
            nearest_matrix = np.column_stack(
                (nearest, [y[j] for j in nearest]))
            nearest_hits = np.array(
                list(filter(lambda row: row[1] == nearest_matrix[0, 1],
                            nearest_matrix)))
            nearest_misses = np.array(
                list(filter(lambda row: row[1] != nearest_matrix[0, 1],
                            nearest_matrix)))
        ...
        # TODO: this sometimes fails when all class labels are different or
        #  when k != 1 is used
    ...
    def _fit(self, X, y):
        ...
        # range_vec = np.array(self.max_diff(X)) # TODO: probably should
        #  convert to float64 and add division by zero check
        range_vec = np.array(self._max_diff(X)).astype('float64')
        ...
        for feature_index in new_features:
            ...
            attr_diff_hits = np.abs(hit_neighbors - hit_values) * \
                one_over_range[feature_index]
            attr_diff_misses = np.abs(miss_neighbors - miss_values) * \
                one_over_range[feature_index]
