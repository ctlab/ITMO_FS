...
class FilterWrapperHybrid(BaseTransformer):
    """
        Performs the filter + wrapper hybrid algorithm by first running the
        ...
        ----------
        filter_ : object
            A feature selection model that should have a fit(X, y) method and
            a selected_features_ attribute available after fitting.
        wrapper : object
            A feature selection model that should have a fit(X, y) method,
            selected_features_ and best_score_ attributes available after
            fitting and a predict(X) method.
        ...
        but it is recommended to use a fast algorithm first to remove a lot
        of unnecessary features before processing the resulting dataset with a
        more time-consuming algorithm (e.g. a wrapper).
        ...
        >>> wrapper = BackwardSelection(LogisticRegression(), 5,
        ... measure='f1_macro')
