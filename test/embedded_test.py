...
class TestCases(unittest.TestCase):
    ...
    def test_MOSS(self):
        # MOSS
        res = MOS(model=SGDClassifier(), weight_func=weight_func, sampling=True).fit_transform(self.data, self.target)
        assert self.data.shape[0] == res.shape[0]
        print("MOSS:", self.data.shape, '--->', res.shape)

    def test_MOSNS(self):
        # MOSNS
        res = MOS(model=SGDClassifier(), weight_func=weight_func, sampling=False).fit_transform(self.data, self.target)
        assert self.data.shape[0] == res.shape[0]
        print("MOSNS:", self.data.shape, '--->', res.shape)

    def test_losses(self):
        for loss in ['log', 'hinge']:
            res = MOS(model=SGDClassifier(), weight_func=weight_func, loss=loss).fit_transform(self.data, self.target)
            assert self.data.shape[0] == res.shape[0]
    ...
    def test_pipeline(self):
        # FS
        p = Pipeline([('FS1', MOS(model=SGDClassifier(), weight_func=weight_func))])
        p.fit(self.data, self.target)
        res = p.transform(self.data)
        assert self.data.shape[0] == res.shape[0]

        # FS - estim
        p = Pipeline([('FS1', MOS(model=SGDClassifier(), weight_func=weight_func)), ('E1', LogisticRegression())])
        p.fit(self.data, self.target)
        assert 0 <= p.score(self.data, self.target) <= 1

        # FS - FS
        p = Pipeline([('FS1', MOS(model=SGDClassifier(), weight_func=weight_func, loss='log')), 
            ('FS2', MOS(model=SGDClassifier(), weight_func=weight_func, loss='hinge'))])
        p.fit(self.data, self.target)
        res = p.transform(self.data)
        assert self.data.shape[0] == res.shape[0]
        ...
        # FS - FS - estim
        p = Pipeline([('FS1', MOS(model=SGDClassifier(), weight_func=weight_func, loss='log')), 
            ('FS2', MOS(model=SGDClassifier(), weight_func=weight_func, loss='hinge')), ('E1', LogisticRegression())])
        p.fit(self.data, self.target)
        assert 0 <= p.score(self.data, self.target) <= 1

    def test_est(self):
        moss = MOS(model=SGDClassifier(), weight_func=weight_func, sampling=True)
        mosns = MOS(model=SGDClassifier(), weight_func=weight_func, sampling=False)
