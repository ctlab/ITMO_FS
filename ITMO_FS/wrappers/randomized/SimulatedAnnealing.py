...
class SimulatedAnnealing(BaseWrapper):
    """
        ...
        init_number_of_features : int
            The number of features to initialize start features subset with,
            by default 5-10 percents of features is used.
        cv : int
    ...
    def _fit(self, X, y):
        ...
        if self.init_number_of_features is None:
            ...
            init_number_of_features = int(self.n_features_ * percentage /
                100) + 1
        else:
            init_number_of_features = self.init_number_of_features

        feature_subset = np.unique(rng.integers(0, self.n_features_,
            init_number_of_features))
        prev_score = cross_val_score(self._estimator,
            X[:, feature_subset], y, cv=self.cv,
        ...
        for i in range(self.iteration_number):
            ...
            if operation == 1 & feature_subset.shape[0] != self.n_features_:
                ...
                include_number = min(not_included_features.shape[0],
                    int(self.n_features_ * (percentage / 100)) + 1)
                cur_subset = np.append(feature_subset, rng.choice(
                    not_included_features, size=include_number, replace=False))
            else:
                # exc
                exclude_number = min(feature_subset.shape[0] - 1,
                    int(self.n_features_ * (percentage / 100)) + 1)
                cur_subset = np.delete(feature_subset, rng.choice(
                    np.arange(feature_subset.shape[0]), size=exclude_number,
                    replace=False))
            cur_score = cross_val_score(self._estimator, X[:, cur_subset], y,
