...
class VDM(BaseTransformer):
    ...
    def _fit(self, X, y=None, **kwargs):
        ...
        vdm = np.zeros(
            (n_samples, n_samples), dtype=np.double
        ...
        for feature in X.T:  # For each attribute:
            ...
            amounts_x = np.array(
                list(map(len, entries_x))
            )  # Array containing amounts of samples
            # for every feature value

            # Calculating deltas:

            deltas = np.zeros(
                (n_values, n_values), dtype=np.double
            )  # Array for calculating deltas

            # Calculating components where exactly one of probabilities is
            # not zero:
            for c in range(n_labels):  # For each class:
                entries = np.array(list(entries_c_x[c].keys()))  # Feature values
                # which are presented in pairs for
                # the class
                amounts = np.array(
                    list(entries_c_x[c].values())
                )  # Corresponding amounts
                non_entries = np.arange(
                    n_values
                )  # Feature values which are not presented in pairs for the class
                ...
                for i in range(len(entries)):  # For each feature value
                    ...
                    v_c_instances = amounts[
                        i
                    ]  # Amount of instances with such value and such class
                    v_instances = amounts_x[
                        value
                    ]  # Amount of instances with such value
                    target_x, target_y = cartesian(
                        [value], non_entries
                    )  # Target indexes for deltas array
                    deltas[target_x, target_y] += (v_c_instances / v_instances) ** 2
            deltas += (
                deltas.T
            )  # As we didn't determined indexes order, for each i, j some components are
            ...
            for c in range(n_labels):  # For each class:
                entries = np.array(
                    list(entries_c_x[c].keys())
                )  # Feature values which are presented in pairs for
                # the class
                amounts = np.array(
                    list(entries_c_x[c].values())
                )  # Corresponding amounts
                ...
                target_x, target_y = cartesian(
                    np.arange(len(entries)), np.arange(len(entries))
                )  # Target indexes
                # for deltas array
                deltas[entries[target_x], entries[target_y]] += (
                    probs[target_x] - probs[target_y]
                ) ** 2
            ...
            if not self.weighted:  # If non-weighted version of metrics was selected
                for i in range(n_values):  # For each value i
                    for j in range(n_values):  # For each value j
                        if (
                            amounts_x[i] == 0 or amounts_x[j] == 0
                        ):  # If some value does not appear in current feature,
                            # skip it
                            continue
                        vdm[cartesian(entries_x[i], entries_x[j])] += deltas[i][j]
            else:  # If weighted version of metrics was selected
                weights = np.zeros(
                    n_values, dtype=np.double
