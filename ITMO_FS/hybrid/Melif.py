...
class Melif(BaseTransformer):
    ...
    def __init__(self, estimator, cutting_rule, filter_ensemble, scorer=None, test_size=0.3, 
        delta=0.5, points=None, verbose=False, seed=42):  # TODO scorer name
    ...
    def _fit(self, X, y):
        ...
        self.__filter_weights = np.ones(len(self.__ensemble)) / len(self.__ensemble)
        self.best_score_ = 0
        self.best_point_ = []
        self.best_f_ = {}
        ...
        if self.verbose:
            ...
            print("Optimizer greedy search, optimizing measure is {}".format(self.scorer))
            time = dt.datetime.now()
            print("time:{}".format(time))

        check_cutting_rule(self.cutting_rule)
        self._train_x, self._test_x, self._train_y, self._test_y = train_test_split(X, y, test_size=self.test_size, random_state=self.seed)
        ...
        n = dict(zip(nu.keys(), self.__measure(np.array(list(nu.values())), best_point_)))
        ...
        if self.verbose:
            ...
            print('Top features:')
            for key, value in sorted(self.best_f_.items(), key=lambda x: x[1], reverse=True):
    ...
    def __search(self, points, features):
        ...
        while i < len(points):
            ...
            self.estimator_.fit(self._train_x[:, self.selected_features_], self._train_y)
            predicted = self.estimator_.predict(self._test_x[:, self.selected_features_])
    ...
    def __score_features(self, nu, candidate):
        ...
        self.estimator_.fit(self._train_x[list(scores.keys())], self._train_y)
        return self.scorer(self._test_y, self.estimator_.predict(self._test_x[keys]))
