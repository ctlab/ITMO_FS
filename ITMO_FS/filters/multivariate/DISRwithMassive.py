...
def _complementarity(x_i, x_j, y):
    return entropy(x_i) + entropy(x_j) + entropy(y) - entropy(
        list(zip(x_i, x_j))) - \
           entropy(list(zip(x_i, y))) - entropy(list(zip(x_j, y))) + entropy(
        list(zip(x_i, x_j, y)))


def _chained_information(x_i, x_j, y):
    return mutual_information(
        x_i, y) + mutual_information(x_j, y) + _complementarity(x_i, x_j, y)
...
[3, 1, 3, 1, 4],[4, 4, 3, 1, 5]], dtype = np.integer)
    ...
    def _fit(self, x, y):
        ...
        while len(self.selected_features_) != self.n_features:
            min_index = np.argmin(
                np.vectorize(
                    lambda a: self.__count_weight(a))(
        ...
        while change:
            ...
            if max_difference > 0:
                ...
                self.selected_features_ = np.append(
                    self.selected_features_, new_selected)
                self.selected_features_ = np.delete(
                    self.selected_features_, new_free)
