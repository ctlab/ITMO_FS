...
from ...utils.information_theory import entropy, joint_entropy, mutual_information
...
class DISRWithMassive(BaseTransformer):
    ...
    def _fit(self, x, y):
        ...
        while len(self.selected_features_) != self.n_features:
            ...
            free_features = np.append(free_features, self.selected_features_[min_index])
            self.selected_features_ = np.delete(self.selected_features_, min_index)

        getLogger(__name__).info(
            "Selected set: %s, free set: %s", self.selected_features_, free_features
        )

        while True:
            selected_weights = np.sum(
                self._edges[np.ix_(self.selected_features_, self.selected_features_)],
                axis=0,
            )
            getLogger(__name__).info("Graph of selected set: %s", selected_weights)

            free_weights = np.sum(
                self._edges[np.ix_(self.selected_features_, free_features)], axis=0
            )
            getLogger(__name__).info(
