...
def VDM(x, y, weighted=True):
    ...
    for column in x.T:  # For each attribute separately:
        ...
        for index, i in enumerate(column):  # For each sample
            count_x.setdefault(i, 0)
            count_x[i] += 1  # Increasing number of samples with attribute value == i

            c = y[index][0]  # Class label c of the corresponding sample
        ...
        for i, i_count_c in count_x_c.items():  # For each attribute value i with its distribution i_count_c
            for j, j_count_c in count_x_c.items():  # For each attribute value j with its distribution j_count_c
                delta = 0
